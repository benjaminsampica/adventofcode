using static _2023.SpokenDigitFinder;
...
{
    [Test]
    public void Given_1abc2_ThenReturns12()
    {
        Assert.That(new Trebuchet().Decode(["1abc2"]), Is.EqualTo(12));
    }

    [Test]
    public void Given_pqr3stu8vwx_ThenReturns38()
    {
        Assert.That(new Trebuchet().Decode(["pqr3stu8vwx"]), Is.EqualTo(38));
    }

    [Test]
    public void Given_a1b2c3d4e5f_ThenReturns15()
    {
        Assert.That(new Trebuchet().Decode(["a1b2c3d4e5f"]), Is.EqualTo(15));
    }

    [Test]
    public void Given_treb7uchet_ThenReturns77()
    {
        Assert.That(new Trebuchet().Decode(["treb7uchet"]), Is.EqualTo(77));
    }

    [Test]
    public void GivenThePuzzleInput_ThenReturnsTheAnswer()
    {
        var d = new string[] { "dqfournine5four2jmlqcgv",
             "7ggzdnjxndfive",
             "twofive4threenine",
        ...
        var answer = new Trebuchet().Decode(d);

        Console.WriteLine(answer);
    }
}

public class DayOne_Two
{

    [Test]
    public void Given_two1nine_ThenReturns29()
    {
        Assert.That(new Trebuchet().Decode(["two1nine"]), Is.EqualTo(29));
    }

    [Test]
    public void Given_eightwothree_ThenReturns83()
    {
        Assert.That(new Trebuchet().Decode(["eightwothree"]), Is.EqualTo(83));
    }

    [Test]
    public void Given_abcone2threexyz_ThenReturns13()
    {
        Assert.That(new Trebuchet().Decode(["abcone2threexyz"]), Is.EqualTo(13));
    }

    [Test]
    public void Given_xtwone3four_ThenReturns24()
    {
        Assert.That(new Trebuchet().Decode(["xtwone3four"]), Is.EqualTo(24));
    }

    [Test]
    public void Given_4nineeightseven2_ThenReturns42()
    {
        Assert.That(new Trebuchet().Decode(["4nineeightseven2"]), Is.EqualTo(42));
    }

    [Test]
    public void Given_zoneight234_ThenReturns14()
    {
        Assert.That(new Trebuchet().Decode(["zoneight234"]), Is.EqualTo(14));
    }

    [Test]
    public void Given_7pqrstsixteen_ThenReturns76()
    {
        Assert.That(new Trebuchet().Decode(["7pqrstsixteen"]), Is.EqualTo(76));
    }
...
{
    public int Decode(string[] inputs)
    {
        var digitSet = new List<int>();
        foreach (var input in inputs)
        {
            var spokenDigitFinder = new SpokenDigitFinder(
            [
                new SpokenDigit("one", 1),
                new SpokenDigit("two", 2),
                new SpokenDigit("three", 3),
                new SpokenDigit("four", 4),
                new SpokenDigit("five", 5),
                new SpokenDigit("six", 6),
                new SpokenDigit("seven", 7),
                new SpokenDigit("eight", 8),
                new SpokenDigit("nine", 9),
            ]);

            var digits = new List<int>();
            foreach (var character in input.Select((c, i) => (c, i)))
            {
                if (char.IsDigit(character.c))
                {
                    digits.Add(int.Parse(character.c.ToString()));
                }
                else
                {
                    var spokenDigit = spokenDigitFinder.FindDigit(character.i, input);
                    if (spokenDigit is not null)
                        digits.Add(spokenDigit.Value);
                }
            }
            ...
            digitSet.Add(int.Parse(firstDigit.ToString() + lastDigit.ToString()));
        }
...
public class SpokenDigitFinder(List<SpokenDigit> SpokenDigits)
{
    private readonly List<SpokenDigit> _spokenDigits = SpokenDigits;

    public int? FindDigit(int currentIndex, string input)
    {
        foreach (var spokenDigit in _spokenDigits)
        {
            var digit = spokenDigit.FindDigit(currentIndex, input);
    ...
    public class SpokenDigit(string Name, int Digit)
    {
        private readonly string _name = Name;
        private readonly int _digit = Digit;
        private int _lastIndex;

        public int? FindDigit(int currentIndex, string input)
        {
            var endIndex = currentIndex + 1;
            var stringToSearch = input[_lastIndex..endIndex];

            if (stringToSearch.Contains(_name))
            {
